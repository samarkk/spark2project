...
object SparkJSON extends App {
  ...
  println("Displaying events dataset data")
  eventsDS.show(false)
  ...
  println("Displaying events data frame created from json data")
  ...
  println("json data frame field projection using get_json_object")
  jsDF.show(false)

  val devicesDF = eventsDS.select(from_json($"device", jsonSchema) as "devices")
    .select($"devices.*")
    .filter($"devices.temp" > 10 and $"devices.signal" > 15)
  println("using from_json to select fields of json data frame")
