...
object StocksMGWSTupleEvent extends App {
  val spark = SparkSession
    .builder
    .appName("StocksMapGroupsWithStateTupleEventTimeout")
    .master("local[*]")
    .getOrCreate()
  ...
  // for event time out we need to have a watermark on a timestamped column
  val stocksWWM = stocks.withWatermark("ts", "10 seconds")
  ...
  val stockUpdates = stocksWWM.groupByKey(x => x.stockId)
    .mapGroupsWithState[StockState, StockUpdateT](
      GroupStateTimeout.ProcessingTimeTimeout) {
        case (stockId: (String, String), events: Iterator[StockEventT], state: GroupState[StockState]) =>
          ...
          // with event time out we cannot set a timeout duration
          // state will not have a time out - it will be controlled by event time
