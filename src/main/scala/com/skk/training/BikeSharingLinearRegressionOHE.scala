...
object BikeSharingLinearRegressionOHE extends App {
  ...
  val fileLoc = "hour.csv"
  val df = spark.read.format("csv").option("header", "true").load(fileLoc)
  ...
  val df2 = df1.withColumn("season", df1("season").cast("double"))
    ...
    .withColumn("hr", df1("hr").cast("double").cast("double"))
  ...
  val encoder = new OneHotEncoderEstimator().setInputCols(
    Array("season", "yr", "mnth", "hr", "holiday",
      "weekday", "workingday", "weathersit")).setOutputCols(
      Array("seasonVec", "yrVec", "mnthVec", "hrVec", "holidayVec",
        "weekdayVec", "workingdayVec", "weathersitVec"))
  //  val encoder = new OneHotEncoderEstimator().setInputCols(
  //    Array("season", "yr", "mnth", "hr")).setOutputCols(
  //      Array("seasonVec", "yrVec", "mnthVec3", "hrVec4"))
  val ohemodel = encoder.fit(df2)
  val oheEncoded = ohemodel.transform(df2)
  val df3 = oheEncoded.drop("season", "yr", "mnth", "hr", "holiday",
    "weekday", "workingday", "weathersit")
  val featureCols = df3.drop("label").columns
  ...
  val Array(training, test) = df3.randomSplit(Array(0.8, 0.2), seed = 12345)
  ...
  val pipelineGrid = new Pipeline().setStages(Array(vectorAssembler, lr_grid))
  ...
  val blinModel = bestPipelineModel.stages(1).asInstanceOf[org.apache.spark.ml.regression.LinearRegressionModel]
  blinModel.extractParamMap()

  blinModel.explainParams.foreach(print)
  ...
  val pipelineGLR = new Pipeline().setStages(Array(vectorAssembler, glr))

  val glrPipelineModel = pipelineGLR.fit(training)

  val glrModel = glrPipelineModel.stages(1).asInstanceOf[org.apache.spark.ml.regression.GeneralizedLinearRegressionModel]
